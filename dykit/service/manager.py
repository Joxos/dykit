...
class ServiceManager:
    ...
    def create(
        ...
        # Determine dykit path
        dykit_path = shutil.which("dykit")
        if not dykit_path:
            dykit_path = f"{sys.executable} -m dykit"
    ...
    def list(self) -> list[dict[str, str]]:
        """List all dykit-managed systemd user services.
        ...
        for path in service_files:
            # Read file and check if it's a dykit service
            try:
                with open(path, "r") as f:
                    content = f.read()
                    if "dykit collect" not in content:
                        continue  # skip non-dykit services
            except OSError:
    ...
    def reload(self, service_name: str) -> None:
        """Reload service unit changes with restart fallback.

        Args:
            service_name: Name of the service to reload (without .service suffix).

        Raises:
            RuntimeError: If daemon-reload fails or both reload and restart fail.
        """
        result = self._systemctl(["daemon-reload"])
        if result.returncode != 0:
            raise RuntimeError(f"daemon-reload failed: {result.stderr}")

        result = self._systemctl(["reload", f"{service_name}.service"])
        if result.returncode == 0:
            click.echo(f"✓ Service {service_name} reloaded")
            return

        result = self._systemctl(["restart", f"{service_name}.service"])
        if result.returncode != 0:
            raise RuntimeError(f"reload/restart failed: {result.stderr}")
        click.echo(f"✓ Service {service_name} restarted (reload not supported)")
